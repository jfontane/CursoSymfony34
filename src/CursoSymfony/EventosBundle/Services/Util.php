...
class Util {
    ...
    public static function slugify($cadena) {
        ...
        $value = self::withoutAccents($cadena);
        ...
        $value = strtolower(preg_replace('/[^A-Z^a-z^0-9^\/]+/', '-', preg_replace('/([a-z\d])([A-Z])/', '\1_\2', preg_replace('/([A-Z]+)([A-Z][a-z])/', '\1_\2', preg_replace('/::/', '/', $value)))));

        return trim($value, '-');
    }

    public static function autoLinkText($text) {
        $text = preg_replace('/<a[^>]+href[^>]+>/', '', str_replace('</a>', '', $text));
        $pattern = "/(((http[s]?:\/\/)|(www\.))(([a-z][-a-z0-9]+\.)?[a-z]"
                . "[-a-z0-9]+\.[a-z]+(\.[a-z]{2,2})?)\/?[a-z0-9.,_\/~#&=:"
                . ";%+?-]+[a-z0-9\/#=?]{1,1})/is";
        $text = preg_replace($pattern, "<a href='$1' class='autolink'>$1</a>", $text);
